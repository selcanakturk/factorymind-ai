import type { RULObservation } from "../types/api";

export const RUL_CSV_HEADERS = [
  "cycle",
  ...
  "sensor_20",
  "sensor_21",
] as const satisfies readonly (keyof RULObservation)[];

export const RUL_MEASUREMENT_COUNT = RUL_CSV_HEADERS.length - 1;

const FORBIDDEN_HEADERS = new Set([
  "rul", "raw_rul", "capped_rul", "target", "target_label", "failure",
  "failure_label", "machine_failure", "predicted_rul", "predicted_rul_cycles",
]);

export interface ParsedRulCsv {
  observations: RULObservation[];
  filename: string;
}

export function parseRulCsv(text: string, filename: string): ParsedRulCsv {
  ...
  if (forbidden.length) throw new Error(`Remove forbidden target or RUL column${forbidden.length > 1 ? "s" : ""}: ${forbidden.join(", ")}.`);
  const missing = RUL_CSV_HEADERS.filter((header) => !headers.includes(header));
  if (missing.length) throw new Error(`Missing required CSV column${missing.length > 1 ? "s" : ""}: ${missing.join(", ")}.`);
  const unknown = headers.filter((header) => !RUL_CSV_HEADERS.includes(header as keyof RULObservation));
  if (unknown.length) throw new Error(`Unknown CSV column${unknown.length > 1 ? "s" : ""}: ${unknown.join(", ")}. Only the required model-input columns are accepted.`);
  ...
  for (let index = 1; index < observations.length; index += 1) {
    ...
    if (current !== previous + 1) throw new Error(`CSV cycles must be consecutive; expected cycle ${previous + 1} after ${previous}.`);
  }
