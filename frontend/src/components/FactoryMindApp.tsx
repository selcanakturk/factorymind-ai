...
import type { AnomalyModelInfoResponse, HealthResponse, ModelInfoResponse, RULModelInfoResponse } from "../types/api";
...
type Page = "overview" | "analysis" | "rul" | "anomaly" | "model";
...
export function FactoryMindApp() {
  ...
  const loadPlatformData = useCallback(async () => {
    ...
    try {
      const [healthResult, modelResult, rulModelResult, anomalyModelResult] = await Promise.all([
        factoryMindApi.health(), factoryMindApi.modelInfo(), factoryMindApi.getRulModelInfo(), factoryMindApi.getAnomalyModelInfo(),
      ]);
      setHealth(healthResult); setModelInfo(modelResult); setRulModelInfo(rulModelResult); setAnomalyModelInfo(anomalyModelResult);
    } catch (caught) {
      setHealth(null);
  ...
  useEffect(() => {
    let active = true;
    Promise.all([factoryMindApi.health(), factoryMindApi.modelInfo(), factoryMindApi.getRulModelInfo(), factoryMindApi.getAnomalyModelInfo()])
      .then(([healthResult, modelResult, rulModelResult, anomalyModelResult]) => {
        if (!active) return;
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
        ...
        <nav aria-label="Primary navigation">
          ...
          <button className="disabled-nav" disabled><span>—</span>Quality Inspection<small>SOON</small></button>
        </nav>
      ...
      <main id="main-content" className="main-content">
        {page === "overview" && <OverviewPage health={health} modelInfo={modelInfo} loading={loading} error={error} onOpenAnalysis={() => navigate("analysis")} onOpenRul={() => navigate("rul")} onOpenAnomaly={() => navigate("anomaly")} onRetry={loadPlatformData} />}
        ...
        {page === "model" && <ModelInfoPage modelInfo={modelInfo} rulModelInfo={rulModelInfo} anomalyModelInfo={anomalyModelInfo} loading={loading} error={error} onRetry={loadPlatformData} />}
